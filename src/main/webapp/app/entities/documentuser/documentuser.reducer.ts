...
const apiUrl = 'api/documents/user';

// Actions

export const getEntities = createAsyncThunk('documentuser/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiUrl}?cacheBuster=${new Date().getTime()}`;
  return axios.get<IDocument[]>(requestUrl);
});

export const getEntity = createAsyncThunk(
  'documentuser/fetch_entity',
...
export const partialUpdateEntity = createAsyncThunk(
  'documentuser/partial_update_entity',
  async (entity: IDocument, thunkAPI) => {
...
export const DocumentUserSlice = createEntitySlice({
  name: 'documentuser',
  initialState,
...
export const { reset } = DocumentUserSlice.actions;
